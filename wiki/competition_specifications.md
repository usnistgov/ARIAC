...
- MOs can be set in any of the four aisles (see figure) as shown in the figure below and will go back and forth in the aisles where they are set.

- The ARIAC environment may contain 0, 1, or 2 MOs.

- Specifications for MOs are as follows:

  - Each MO will have a travel time of 9 s (from one end of the aisle to the other end).

  - Each MO will have a wait time of 7 s when it reaches the end of the aisle.

  - At the beginning of the simulation each MO will wait 16 s before moving.

    - **Note**: When two MOs are placed in the environment on the same side you may see one of them starting to move as soon as the simulation starts to reach the other side (travel time = 9 s), then waits 7 s before starting moving.
