  ...
  // the movable kit tray and products in the movable kit tray
    ...
    if (_sdf->HasElement("lock_unlock_kt1_topic")) {
        ...
        this->lock_unlock_kt1_gz_sub =
            this->gzNode->Subscribe(lock_unlock_service_str,
                &MovableKitTrayPlugin::LockUnlockServiceCallback,
                this);
    }
    else {
        ...
        if (_sdf->HasElement("get_content_dwood_1")) {
            ...
            this->movable_tray_content_srv =
                this->rosNode->advertiseService(
                    content_service,
                    &MovableKitTrayPlugin::HandleMovableTrayContent, this);
        }
    }
...
void MovableKitTrayPlugin::ProcessContactingModels()
{
    ...
    for (auto model : this->contactingModels) {
        ...
        if (model->GetName().compare("tray_table1") != 0 &&
            model->GetName().compare("tray_table2") != 0) {
        ...
        auto model_it = std::find(this->grippable_model_types.begin(),
            this->grippable_model_types.end(), object_type);
...
void MovableKitTrayPlugin::LockContactingModels()
{
    ...
    for (auto part : this->contactingModels) {
        ...
        // make sure this model is an ariac part
        auto it = std::find(this->grippable_model_types.begin(),
...
bool MovableKitTrayPlugin::HandleMovableTrayContent(
    ros::ServiceEvent<nist_gear::DetectKittingShipment::Request,
    nist_gear::DetectKittingShipment::Response>& event)
{
